...
**A refresh costs two round trips, not one.** Filling buyer identity needs a *storefront* customer access token, minted by the Customer Account API mutation `storefrontCustomerAccessTokenCreate`, whose own description says to renew it "each time you update the Customer Access Token found in the Authorization header." So a refresh is the token endpoint plus that mutation. Its payload carries no expiry field, so nothing observable keys its renewal — the coupling to the refresh cycle is the only signal there is.
...
**Concurrent requests can race.** Two requests from one browser can both reach the boundary with the same expired token and both present the same refresh token. Hydrogen's single-flight guard is an in-closure `Map`, per-instance and per-process, keyed on the *old* refresh token — no help across isolates, and a cookie store shares no state at all. Whether Shopify rotates refresh tokens at all decides how bad this is, and it is unverified: [#40](https://github.com/KookiKodes/solidifront/issues/40).
